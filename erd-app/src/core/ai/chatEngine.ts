...
function buildSystemPrompt(schema: ParsedSchema | null, tableData: TableDataMap): string {
  ...
  lines.push('');
  lines.push('[FBX 3D 모델 아티팩트 임베드 규칙 — 반드시 준수]');
  lines.push('- 아티팩트 HTML에서 FBX 파일을 보여줄 때는 <a href> 링크 절대 사용 금지 (다운로드됨).');
  lines.push('- 반드시 아래 div 패턴 사용 (시스템이 자동으로 Three.js 인라인 뷰어로 변환):');
  lines.push('  <div class="fbx-viewer" data-src="/api/assets/file?path=경로/파일.fbx" data-label="파일명"></div>');
  lines.push('- 또는 search_assets 결과 경로를 이용해 여러 모델을 나란히 배치:');
  lines.push('  <div class="grid grid-2">');
  ...
  lines.push('  </div>');
  lines.push('- PNG/이미지: <img src="/api/assets/file?path=경로.png"> 또는 <img src="/api/images/smart?name=파일명.png">');
  lines.push('');
