...
export const TOOL_META: ToolMeta[] = [
  ...
  { name: 'query_string_table',    label: '스트링 테이블 검색',   emoji: '🌍', dataSources: ['strings'] },
];
...
csv: 완전히 새로운 데이터만. rows: 1~2행 소량 전용 (3행 이상 금지).`,
  ...
  },
  {
    ...
    description: '로컬라이징 스트링 테이블(Google Sheets) 검색. 키 이름이나 번역 텍스트로 검색. DB 컬럼 값이 스트링 키(예: Skill_Desc_1100)인 경우 실제 번역 텍스트 확인 가능.',
    input_schema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: '검색어 (스트링 키 또는 번역 텍스트)' },
        language: { type: 'string', description: '언어 필터 (생략 시 전체)' },
      },
      required: ['query'],
    },
  },
...
const TOOL_GROUPS: Record<string, { tools: string[]; keywords: RegExp }> = {
  ...
  string: {
    ...
    keywords: /스트링|string|번역|로컬라이|locali[zs]|언어|language|다국어|i18n|한국어|영어|텍스트\s*키|키\s*값|string\s*table|난사|이름.*뭐|뭐.*이름/i,
  },
};
...
function buildSystemPrompt(
  ...
  if (hasTools(['query_string_table'])) {
    ...
    lines.push('');
    lines.push('### DB ↔ 스트링 테이블 매핑');
    lines.push('DB 컬럼 중 name_id, desc_id, _Name, _Desc 등은 스트링 테이블 키(예: Skill_Desc_1100)를 담고 있음.');
    lines.push('query_game_data 결과에 스트링 키가 포함되면 자동으로 한국어 번역이 부착됨 (📝 스트링 키 번역 섹션).');
    lines.push('사용자가 "난사", "연속 사격" 같은 한국어 텍스트로 검색 시:');
    lines.push('  1. query_string_table(query="난사")로 스트링 키를 먼저 찾고');
    lines.push('  2. 찾은 키(예: Skill_Name_1100)로 query_game_data에서 DB 검색');
    lines.push('이 2단계 검색으로 번역 텍스트 → DB 데이터 역추적 가능.');
    lines.push('');
  }
  lines.push('');
  lines.push('## SQL 주의');
...
위 방식으로 각 연결 테이블마다 탭을 추가하세요.`;
        ...
        else if (tb.name === 'query_string_table') {
          const t0 = performance.now();
          try {
            const resp = await fetch('/api/tool/execute', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ tool: 'query_string_table', input: inp }),
            });
            if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
            const data2 = await resp.json() as { result: string; data?: { total: number; matched: number; displayed: number; headers: string[] } };
            const duration = performance.now() - t0;
            resultStr = data2.result;
            tc = { kind: 'knowledge', action: 'read', name: `스트링 검색: ${String(inp.query ?? '')}`, content: resultStr, duration } as KnowledgeResult;
          } catch (e) {
            resultStr = `스트링 테이블 조회 오류: ${e instanceof Error ? e.message : String(e)}`;
            tc = { kind: 'knowledge', action: 'read', name: '스트링 테이블', content: resultStr, error: resultStr } as KnowledgeResult;
          }
        }
