...
export function renderCanvas(
  ...
  if (focusProgress > 0) {
    ...
    if (isDark) {
      grad.addColorStop(0, `rgba(18, 12, 55, ${focusProgress * 0.45})`);
      grad.addColorStop(1, `rgba(6, 4, 28, ${focusProgress * 0.72})`);
    } else {
      grad.addColorStop(0, `rgba(100, 80, 230, ${focusProgress * 0.05})`);
      grad.addColorStop(1, `rgba(60, 40, 180, ${focusProgress * 0.12})`);
    }
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, width, height);
  }
